...
class MultiHeadDiffAttention(nn.Module): #-! where depth is layer index or lambda_init = 0.8 for all layers
...
class FFT(nn.Module):
  def __init__(self, hidden_channels, filter_channels, n_heads, n_layers=1, kernel_size=1, p_dropout=0.,
    ...
    for i in range(self.n_layers):
      self.self_attn_layers.append(
        MultiHeadDiffAttention(hidden_channels, hidden_channels, n_heads, i+1, p_dropout=p_dropout, proximal_bias=proximal_bias,
                           proximal_init=proximal_init))
      self.norm_layers_0.append(LayerNorm(hidden_channels))
      self.ffn_layers.append(
        FFN(hidden_channels, hidden_channels, filter_channels, kernel_size, p_dropout=p_dropout, causal=True))
